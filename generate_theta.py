...
#%% Packages to import
import argparse
from utils_func import set_params, f_exist, folder_create, load_obj, save_obj, load_lfw, random_pairs, suppress_stdout
import numpy as np
...
def augment_lfw(person_range):
    ...
    # Read data
    (imgs, index, landmarks, _) = load_lfw()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-maxp', action="store", dest="max_pairs", default = 10,
                        ...
                        person. Set to 200000 to get all possible combinations''')
    res = parser.parse_args()
    person_range = [int(res.person_range[0]), int(res.person_range[1])]
    ...
    max_person_range = 500  # for some reason it has problems with processing
