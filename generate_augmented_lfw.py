...
#    
    ...
    for i in range(10):
        h5f = h5py.File('datasets/lfw_3/lfw_augment_cpab_cv_' + str(i) + '.h5', 'w')
        ...
        h5f.create_dataset('X_test', ((n_scaling+1)*X_test.shape[0], 2, rows, cols, nchannels), dtype='uint8')
        h5f.create_dataset('y_test', ((n_scaling+1)*y_test.shape[0],), dtype='uint8')
        ...
        for idx in range(X_test.shape[0]):
            ...
            for aug_count in range(1, n_scaling+1):   
                ...
                theta = np.random.multivariate_normal(m_k[cluster_idx].flatten().astype('float64'), 
                                    np.linalg.pinv(v_k[cluster_idx] * W_k[cluster_idx].astype('float64')))
                h5f['X_test'][i+aug_count,0] = generate_new_images(img1, tw, theta, 1, [1,1])[0]
            
                cluster_idx = np.random.choice(range(K), 1, replace=False, p = pri)[0]
                theta = np.random.multivariate_normal(m_k[cluster_idx].flatten().astype('float64'), 
                                    np.linalg.pinv(v_k[cluster_idx] * W_k[cluster_idx].astype('float64')))
                h5f['X_test'][i+aug_count,1] = generate_new_images(img2, tw, theta, 1, [1,1])[0]
            
                h5f['y_test'][i+aug_count] = target
    ...
    #%% CPAB augmentation 2
    print('CPAB augmentation2')
    for i in range(10):
        progressBar(i, 9)
        h5f_old = h5py.File('datasets/lfw_3/lfw_augment_cpab_cv_' + str(i) + '.h5', 'r')
        h5f_new = h5py.File('datasets/lfw_3_down/lfw_augment_cpab_cv_' + str(i) + '.h5', 'w')
