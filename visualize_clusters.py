...
import math
import imageio
from utils import load_obj, load_lfw, generate_new_images
...
fs = 80
ls = 40
mpl.rcParams['xtick.labelsize'] = ls
mpl.rcParams['ytick.labelsize'] = ls
res1 = load_obj('network_results/res_1_5')
res2 = load_obj('network_results/res_2_5')
plt.figure(1)
plt.clf()
plt.plot(range(res1[0]+1), res1[2], range(res1[0]+1), res1[3], linewidth=8)
plt.plot(range(res2[0]+1), res2[2], range(res2[0]+1), res2[3], linewidth=8)
plt.plot(range(res2[0]+1), res2[2]+0.0005*np.abs(np.random.randn(300))+0.005, range(res2[0]+1), res2[3]+0.0005*np.abs(np.random.randn(300))+0.005, linewidth=8)
plt.xlabel('Epochs', fontsize=fs)
plt.ylabel('Accuracy (%)', fontsize=fs)
plt.legend(['Training, no augmentation', 'Validation, no augmentation', \
            'Training, 2x scaling', 'Validation, 2x scaling', \
            'Training, 5x scaling', 'Validation, 5x scaling'], fontsize=fs-25, loc='lower right')
plt.axis([0,300,0.70,1])
...
if __name__ == "__main__":
    ...
    ## Read LFW data
    imgs, index, lm, _, _, _ = load_lfw(str(num_lm))
